 ...
 * ##### 3. Type Safety
 *
 * The compiler enforces that a transactional program `Txn[A]` must be explicitly committed to be transformed into an executable
 * program `F[A]`. This makes it impossible for a defined block of business logic to be accidentally left out of a transaction.
 *
 * ##### 4. Resource Safety
 *
...
trait TransactionManager[F[_], Txn[_]]:
   ...
   *
   * > **Important**: in case the transaction is **rolled back**, these lifted operations will **not be reverted**. For example, a
   * logged message will still be output. It's the programer's responsibility to revert such actions when necessary.
   */
