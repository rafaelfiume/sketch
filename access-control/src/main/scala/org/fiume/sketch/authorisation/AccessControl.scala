...
trait AccessControl[F[_], Txn[_]: Monad] extends Store[F, Txn]:
  ...
  def canAccess[T <: Resource, A](userId: UserId, resourceId: ResourceId[T])(ops: () => Txn[A]): Txn[Either[Unauthorised, A]] =
    canAccess(userId, resourceId).ifM(
      ifTrue = ops().map(Right(_)),
      ifFalse = Left("Unauthorised").pure[Txn]
    )

  def store[T <: Resource](userId: UserId, resourceId: ResourceId[T], role: Role): Txn[Unit]

  def fetchRole[T <: Resource](userId: UserId, resourceId: ResourceId[T]): Txn[Option[Role]]

  def delete[T <: Resource](userId: UserId, resourceId: ResourceId[T]): Txn[Unit]
