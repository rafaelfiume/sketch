...
import org.fiume.sketch.shared.app.{Resource, ResourceId, WithUuid}
...
type UserId = ResourceId[UserEntity]
object UserId:
  def apply(uuid: UUID): UserId = ResourceId[UserEntity](uuid)
sealed trait UserEntity extends Resource
